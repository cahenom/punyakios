...
export default function TopupGames({route}) {
  ...
  } = useTopupProducts(provider, title, '/api/product/games', 'games');
  ...
  const confirmOrder = async () => {
    ...
    console.log('[GAMES DEBUG] Initiating confirmOrder');
    console.log('[GAMES DEBUG] current customer_no state:', customer_no);
    console.log('[GAMES DEBUG] current selectItem state:', JSON.stringify(selectItem, null, 2));
    ...
    try {
      ...
      }, 'Verifikasi sidik jari atau wajah untuk melakukan topup game');
  ...
  return (
    <SafeAreaView style={{flex: 1, backgroundColor: isDarkMode ? DARK_BACKGROUND : WHITE_BACKGROUND, paddingBottom: 100}}>
      <CustomHeader title={title || "Topup Games"} />
      ...
      <View style={[styles.container, {paddingBottom: 10, backgroundColor: isDarkMode ? DARK_BACKGROUND : WHITE_BACKGROUND}]}>
        ...
        <View style={styles.formGroup}>
          <Input
            value={customer_no}
            placeholder="Masukan ID Game/Nickname"
            onchange={text => {
              setCustomerNo(text);
