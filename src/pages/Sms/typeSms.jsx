...
export default function TypeSms({route, navigation}) {
  ...
  const loadTypes = async (forceRefresh = false) => {
    try {
      ...
      setLoading(false);
      console.error('Error in TypeSms loadTypes:', err);
    }
  };


  const handleTypePress = (selectedType) => {
    navigation.navigate('TopupSms', {
      provider: provider,
  ...
  return (
    <SafeAreaView style={{flex: 1, backgroundColor: isDarkMode ? DARK_BACKGROUND : WHITE_BACKGROUND}}>
      <CustomHeader title={title || "Jenis Paket SMS"} />
